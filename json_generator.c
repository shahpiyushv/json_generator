...
int json_gen_str_end(json_gen_str_t *jstr) {
    ...
    if (jstr->buf) {
        *jstr->free_ptr = '\0';
        if (jstr->flush_cb) jstr->flush_cb(jstr->buf, jstr->priv);
    }
    memset(jstr, 0, sizeof(json_gen_str_t));
...
static inline void json_gen_handle_comma(json_gen_str_t *jstr) {
    if (jstr->comma_req) json_gen_add_to_str(jstr, ",");
}
...
int json_gen_push_object_str(json_gen_str_t *jstr, const char *name,
...
int json_gen_push_array_str(json_gen_str_t *jstr, const char *name,
...
int json_gen_obj_set_string(json_gen_str_t *jstr, const char *name,
                            const char *val) {
    json_gen_handle_comma(jstr);
    json_gen_handle_name(jstr, name);
...
int json_gen_obj_start_long_string(json_gen_str_t *jstr, const char *name,
                                   const char *val) {
    json_gen_handle_comma(jstr);
    json_gen_handle_name(jstr, name);
